...
import { createPagamento, type State } from '../actions';
...
export default function CreatePagamentoForm() {
  ...
  const [state, dispatch, pending] = useActionState<State, FormData>(createPagamento, initialState);
    ...
    <form action={dispatch}>
        ...
        <div className="mb-4">
          ...
          >
            ...
            {clientes.map(cliente => (
              <option key={cliente.idClientes} value={Number(cliente.idClientes)}>{cliente.nome}</option>
            ))}
          </select>
      ...
      <div className="mt-6 flex justify-end gap-4">
        ...
        <button type="submit" 
                ...
                disabled={pending}
        >
          {pending ? 'Criando...' : 'Criar Pagamento'}
        </button>
      </div>
    </form>
  );
