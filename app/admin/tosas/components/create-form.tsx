...
import { createTosa, type State } from '../actions';
...
export default function CreateTosaForm({ animais }: { animais: Animal[] }) {
  const initialState: State = { message: null, errors: {}, data: null };
  const [state, dispatch, pending] = useActionState<State, FormData>(createTosa, initialState);

  // Transformar os dados para o formato usado no select
  // Criar uma opção para cada relação animal-tutor, mesmo que o nome do animal se repita
  ...
  const [valorSelecionado, setValorSelecionado] = useState(
    state.data?.Animais_tem_clientes_id_animais_tem_clientes
      ? String(state.data.Animais_tem_clientes_id_animais_tem_clientes)
      : ''
  );
      ...
      <div className="rounded-md bg-gray-50 p-4 md:p-6">
        ...
        <div className="mb-4">
          ...
          <input
            ...
            name="atividade"
            type="text"
            defaultValue={state.data?.atividade || ''}
        ...
        <div className="mb-4">
          ...
          <input
            ...
            type="datetime-local"
            defaultValue={state.data?.data_hora_inicio || ''}
            className="block w-full rounded-md border border-gray-200 py-2 px-3 text-sm outline-2 placeholder:text-gray-500"
        ...
        <div className="mb-4">
          ...
          <input
            ...
            name="data_hora_fim"
            type="datetime-local"
            defaultValue={state.data?.data_hora_fim || ''}
            className="block w-full rounded-md border border-gray-200 py-2 px-3 text-sm outline-2 placeholder:text-gray-500"
